...
describe("things the rls should provide", () => {
  test("should allow delete records as a user", async () => {
    ...
    expect(q1result.rows.length).not.toBe(q2result.rows.length);

    await deleteUser(user.id);
  });

  test("should deny delete records as a user who does not own the record", async () => {
    const user = await signupUser();
  ...
  test.skip("should allow to update records as a user who owns the record", async () => {
    // setup
    const user = await signupUser();
    const sensor = await createSensor({
      user_id: user.id,
  ...
  test("should allow to insert records as a user who owns the sensor", async () => {
    // setup
    const user = await signupUser();
    const sensor = await createSensor({
      user_id: user.id,
    ...
    const response = await fetch(`${supabaseUrl}/rest/v1/records`, {
      method: "POST",
      headers: {
        apikey: supabaseAnonKey,
        "Content-Type": "application/json",
  ...
  test("should fail to insert records as a user who does not owns the sensor", async () => {
    // setup
    const user = await signupUser();
    const sensor = await createSensor({
      user_id: user.id,
